...
> 📰 **ARIS-Code v0.4.7** (2026-05-16) — **DashScope Coding Plan 405 fixed** ([#159](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/159)) via `native-tls` switch ([#225](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/225)) | **`reasoning_content` replay for all reasoning models** (OpenAI o1/o3/o4 / DeepSeek-R1 / etc.), not just Kimi — pairs with v0.4.5 `reasoning_effort='xhigh'` for coherent multi-turn reasoning ([#226](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/226)) | Cleanup: 600+ lines of `rusty-claude-cli` prototype dead code + unused `rustyline` dep + "Claw Code" → "ARIS-Code" rebranding in user-facing strings | Credit [@GetIT-Sunday](https://github.com/GetIT-Sunday).

> 📰 **ARIS-Code v0.4.6** (2026-05-14) — **🚨 Two long-standing silent bugs fixed**: (1) `PermissionMode::Prompt` was *silently allowing every tool* due to derived-`Ord` bug, now routes through prompter correctly; (2) system prompt hard-coded `current_date = "2026-03-31"`, making models reject post-cutoff real data (incl. users' own arXiv papers) as "future / prompt injection" — now uses real system time. Plus **Custom OpenAI-compatible provider** (`/setup` option 11) with dynamic `/models` discovery — credit [@Anduin9527](https://github.com/Anduin9527) ([#221](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/221) + [#222](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/222)).
>
> 📰 **ARIS-Code v0.4.5** (2026-05-13) — **First-class reasoning-model support** — Thinking content blocks end-to-end (fixes [#161](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/161)) + `reasoning_effort='xhigh'` actually on the wire for GPT-5.5 / o1 / o3 / o4 / DeepSeek-thinking | **DeepSeek V4 Pro** + **Xiaomi MiMo** + **Qwen 3.6** + **Doubao** in `/setup` (options 7-10) | **Claude Code object-style hooks** parser | Default model bumped to **Claude Opus 4.7 + GPT-5.5** | REPL input hardening: multi-line wrap / Cmd+V paste / CJK at wrap boundary | GitHub Actions CI added | Credits: [@GO-player-hhy](https://github.com/GO-player-hhy) ([#186](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/186)), [@Jxy-yxJ](https://github.com/Jxy-yxJ) ([#171](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/171)), [@GetIT-Sunday](https://github.com/GetIT-Sunday) ([#216](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/216) partial)
>
...
- **2026-05-17** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 📦 **Arch C — single-owner helpers move into their owning SKILL** (Phase 3 of the tools-stability roadmap). Three Phase 3 sub-commits land the "self-contained skills + ARIS shared runtime" architecture recommended by the community feedback that triggered Phase 1: `tools/figure_renderer.py` → `skills/figure-spec/scripts/`, `tools/paper_illustration_image2.py` → `skills/paper-illustration-image2/scripts/`, `tools/experiment_queue/{queue_manager,build_manifest}.py` → `skills/experiment-queue/scripts/`. Each owning SKILL.md now resolves via a hybrid chain — Layer 0 prefers `${CLAUDE_SKILL_DIR}/scripts/<helper>` (CC 1.0+ self-contained), Layers 1-3 are the canonical resolver chain via `os.execv` shims kept at the legacy `tools/` paths so existing users continue to work without re-running anything. `shared-references/integration-contract.md` §2 gains a new "Layer 0 — self-contained owner SKILL" subsection codifying the pattern for future single-owner moves. Shared helpers (`research_wiki.py`, `save_trace.sh`, `verify_paper_audits.sh`, fetchers …) stay in `tools/` as the ARIS shared runtime — only true single-owner helpers move. **⚠️ Existing users**: no action needed — legacy `tools/` entries are now `os.execv` shims forwarding to the new canonical location; `.aris/tools/<helper>` symlinks created by `install_aris.sh` continue to resolve transparently through the shims.
- **2026-05-16** — ![FIX](https://img.shields.io/badge/FIX-orange?style=flat-square) 🛠 **Canonical helper resolver migrated across 50+ SKILLs** (Phase 1 of the tools-stability roadmap). Every SKILL.md caller of the 10 canonical helpers (`verify_papers.py`, `extract_paper_style.py`, `verify_paper_audits.sh`, `save_trace.sh`, `research_wiki.py`, `arxiv_fetch.py` + 4 sibling fetchers, `verify_wiki_coverage.sh`) now resolves helpers via the strict-safe 3-layer chain `.aris/tools/` → `tools/` → `$ARIS_REPO/tools/` documented in [`shared-references/integration-contract.md`](skills/shared-references/integration-contract.md) §2, instead of hardcoded `python3 tools/foo.py`. The §2 contract also defines 5 failure policies (A gate / B side-effect / C forensic / D1 cascade / D2 multi-source aggregate / E diagnostic) with POSIX-sh + set-e + set-u safe examples plus a per-helper policy assignment table, so downstream authors do not guess. Codex skill mirror synced. **⚠️ Existing users**: no action needed if `bash tools/install_aris.sh` was rerun any time after 2026-04-30 (Phase 0 creates the `.aris/tools` symlink). Otherwise rerun once — the install is idempotent.
- **2026-05-16** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🤖 **Advisory CI lint for hardcoded `tools/<helper>` references in SKILL.md** ([`.github/workflows/lint-skills-helpers.yml`](.github/workflows/lint-skills-helpers.yml) + [`tools/lint_skills_helpers.sh`](tools/lint_skills_helpers.sh)). New workflow runs on PRs that touch SKILL.md / shared-references, scans for the 14 canonical helper-invocation patterns, and reports any direct `python3 tools/foo.py` or `bash tools/foo.sh` references back in the GitHub Actions summary. **Advisory only — never fails CI**; reviewers can request migration before merge. The current tree has 4 carry-over flags (figure-spec, paper-illustration-image2, two prose mentions) that are intentional and will be resolved in a follow-up self-contained-script move.
...
<details>
<summary>Earlier updates (2026-03-12 — 2026-05-13, 62 entries)</summary>
