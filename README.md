...
> 📰 **ARIS-Code v0.4.8** (2026-05-17) — **Skill helper subsystem rewrite + 4 bug fixes.** Bundled helpers now extract to `~/.config/aris/cache/<version>/` at startup (no more cwd pollution); every Skill tool invocation surfaces a `helperReport` JSON + 4-layer resolver preamble. `/skills export` copies bundled helpers alongside SKILL.md. New `integration-contract.md` with 6 failure policies for SKILL authors. 8 shared cross-skill helpers bundled (arxiv/deepxiv/exa/S2/openalex fetchers + save_trace + verify_papers + verify_paper_audits). `/research-lit` + `/deepxiv` SKILL.md migrated to fallback chain. **Fixes**: (a) `gpt-5.5/o3/o4 + tools` 400 on OpenAI (strips `reasoning_effort` for the gated path, one-shot stderr warning); (b) Custom reviewer reset to gpt-5.5 every restart (`/setup` menu option 9 vs 8 bug + `LlmReview` no longer falls back to gpt-5.5 for Custom); (c) `json error: missing field signature` for third-party Anthropic-compat proxies ([#228](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/228) by [@StevenUST](https://github.com/StevenUST)); (d) `--version` Build date hardcoded `2026-03-31` (v0.4.6 leftover; now build.rs-injected).
