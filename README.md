...
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 💎 **Gemini 3 model aliases now usable in `/gemini-search`** ([SKILL.md](skills/gemini-search/SKILL.md)). If your `gemini-cli` is v0.40+, you can override with `/gemini-search "topic" — model: auto-gemini-3` (auto-routes to the latest Gemini 3 family member: pro / flash / etc.) or `— model: gemini-3-pro-preview` / `gemini-3-flash-preview` for explicit selection. The default stays `gemini-2.5-pro` (broadest compatibility); the new aliases are opt-in. Both `mcp__gemini-cli__ask-gemini` and the CLI fallback accept these aliases verbatim — no skill code change required, just add `— model:` to your invocation
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🛠️ **`/experiment-queue` orchestration paths repaired** ([#193](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/193)). Fixes a user-reported regression where the queue could not find its scheduler / build-manifest helpers at runtime: 4 root causes (wrong `tools/` subdir, no remote `scp` step, `scp + $HOME` SFTP-mode unreliability, `--log` flag silently ignored by `queue_manager.py`) plus 3 audit-surfaced edge cases (manifest never copied to remote, executable `<project>` placeholder, summary-author ambiguity in Step 5). Step 3 now splits into helper resolution / remote-path computation / bootstrap+scp / launch-with-`--log-dir` / persisted `run_meta.txt` / resume-existing-queue. Pure prose + docstring fix; `queue_manager.py` Python logic untouched. Three rounds of Codex MCP `gpt-5.5` xhigh audit caught the cascade
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🔗 **`install_aris.sh` adds optional `.aris/tools` symlink** ([#192](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/192), closes [#174](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/174)). Phase 0 of the four-step tools-stability migration (#174 → #176 → #177 → #178). Install / reconcile creates `.aris/tools -> <aris-repo>/tools`; idempotent, dry-run-aware, and safely skips when user-modified content exists at the target. **Zero impact on existing users** until they rerun the installer; currently inert (no SKILL.md references it yet — that's #177). 8-case regression test in `tests/test_install_aris_tools_symlink.py`. Windows `install_aris.ps1` parallel update tracked as follow-up
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🔬 **Three new opt-in audit flags ship via fast-path delegated-agent workflow** ([#187](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/187), [#188](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/188), [#189](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/189)). [`/citation-audit --uncited`](skills/citation-audit/SKILL.md) surfaces bib entries with no `\cite{}` reference (detect-only, never sent to reviewer). [`/proof-checker --deep-fix`](skills/proof-checker/SKILL.md) augments Phase 1 reviewer prompt with a structured repair-grade plan (corrected_statement / changed_equations / minimal_tex_patch_plan / closure_tests + algebra sanity for Schur / quadratic-form). [`/proof-checker --restatement-check`](skills/proof-checker/SKILL.md) adds Phase 3.6 cross-location theorem drift detector (6 drift signatures: conditional_loss / scope_change / quantifier_loss / regime_envelope_change / constant_change / variable_rename). **Zero behavior change** when flags unset; all advisory (verdict / reason_code / summary untouched). Plus follow-up doc PRs ([#190](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/190) thread-policy clarification, [#191](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/191) auto-loop cross-reference). All built via the new delegated-agent + maintainer-fixup pattern: each PR has author commit + maintainer fixup commit visible in history; reviewer-side Codex MCP audit (gpt-5.5 xhigh) caught 6+ blockers across the series
...
<details>
<summary>Earlier updates (2026-03-12 — 2026-04-20, 44 entries)</summary>
