...
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 💎 **`/gemini-search` 现在可用 Gemini 3 模型别名** ([SKILL.md](skills/gemini-search/SKILL.md))。如果你的 `gemini-cli` 是 v0.40+，可以用 `/gemini-search "topic" — model: auto-gemini-3`（auto-route 到最新 Gemini 3 家族成员：pro / flash / etc.）或 `— model: gemini-3-pro-preview` / `gemini-3-flash-preview` 显式选择。默认仍是 `gemini-2.5-pro`（最广兼容性），新别名是 opt-in。`mcp__gemini-cli__ask-gemini` 和 CLI fallback 都直接接受这些别名——不需要改 skill 代码，只在调用时加 `— model:` 参数即可
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🛠️ **`/experiment-queue` 编排路径修复** ([#193](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/193))。修复用户反馈的 critical bug：跑实验时 queue 找不到 scheduler / build-manifest 路径导致整个 skill 不可用。4 处根因（`tools/` 缺子目录、远程 `scp` 步骤缺失、`scp + $HOME` 在 SFTP 模式下不可靠展开、`--log` flag 被 `queue_manager.py` 静默忽略）+ 3 处审计发现的边界 bug（manifest 没传到 remote、可执行 bash 里有 `<project>` 占位符、Step 5 没说清谁写 summary）。Step 3 现在分成 helper 解析 / remote-path 计算 / bootstrap+scp / launch-with-`--log-dir` / 持久化 `run_meta.txt` / resume 现有 queue。纯 prose + docstring 修订；`queue_manager.py` Python 逻辑未动。3 轮 Codex MCP `gpt-5.5` xhigh review 抓出 cascade
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🔗 **`install_aris.sh` 加可选 `.aris/tools` 符号链接** ([#192](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/192), 关闭 [#174](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/174))。tools-stability 4 步迁移计划（#174 → #176 → #177 → #178）的 Phase 0。Install / reconcile 时创建 `.aris/tools -> <aris-repo>/tools`；幂等、dry-run-aware、目标位置已有用户内容时安全跳过。**对老用户零影响**直到 rerun installer；目前是 inert（没有 SKILL.md 引用它，那是 #177 的事）。`tests/test_install_aris_tools_symlink.py` 8-case 回归。Windows `install_aris.ps1` 平行更新作为 follow-up
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🔬 **三个 opt-in audit flag 通过 fast-path delegated-agent 工作流落地** ([#187](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/187), [#188](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/188), [#189](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/189))。[`/citation-audit --uncited`](skills/citation-audit/SKILL.md) 扫出 bib 里没人 `\cite{}` 的死条目（纯检测、不送 reviewer）。[`/proof-checker --deep-fix`](skills/proof-checker/SKILL.md) 让 Phase 1 reviewer 输出 repair-grade 修复计划（corrected_statement / changed_equations / minimal_tex_patch_plan / closure_tests + Schur/quadratic-form 代数 sanity）。[`/proof-checker --restatement-check`](skills/proof-checker/SKILL.md) 加 Phase 3.6 跨位置定理飘移检测（6 类 drift：conditional_loss / scope_change / quantifier_loss / regime_envelope_change / constant_change / variable_rename）。**flag 不传时零行为变化**；全部 advisory（不改 verdict / reason_code / summary）。同期还合了两条文档 PR（[#190](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/190) thread-policy 措辞 / [#191](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/191) auto-loop cross-reference）。全部走 delegated-agent + maintainer-fixup 工作流：每 PR 都有 author commit + maintainer fixup commit 在历史可见；reviewer 端 Codex MCP audit（gpt-5.5 xhigh）跨整个 series 抓出 6+ 个 blocker
