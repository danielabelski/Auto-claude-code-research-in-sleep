...
- **2026-05-17** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 📦 **Arch C — single-owner helper 迁入对应 SKILL**（tools-stability roadmap Phase 3）。3 个 Phase 3 子提交完成"self-contained skill + ARIS shared runtime"架构（即触发 Phase 1 的社区反馈所建议的 layout）：`tools/figure_renderer.py` → `skills/figure-spec/scripts/`、`tools/paper_illustration_image2.py` → `skills/paper-illustration-image2/scripts/`、`tools/experiment_queue/{queue_manager,build_manifest}.py` → `skills/experiment-queue/scripts/`。每个 owner SKILL.md 改用 hybrid 链：Layer 0 优先 `${CLAUDE_SKILL_DIR}/scripts/<helper>`（CC 1.0+ self-contained），Layer 1-3 是 canonical resolver 链，并在原 `tools/` 路径保留 `os.execv` Python 转发 shim，已有用户**不用重跑任何命令**。`shared-references/integration-contract.md` §2 新增 "Layer 0 — self-contained owner SKILL" 子节，把这个 pattern 固化下来供后续 single-owner 迁移参考。Shared helper（`research_wiki.py`、`save_trace.sh`、`verify_paper_audits.sh`、fetchers ……）继续留在 `tools/` 作为 ARIS shared runtime——只有真正 single-owner 的 helper 才迁。**⚠️ 现有用户**：无需操作；legacy `tools/` 入口现在是 `os.execv` shim 转发到新 canonical 位置，`install_aris.sh` 创建的 `.aris/tools/<helper>` 软链通过 shim 透传，行为一致。
- **2026-05-16** — ![FIX](https://img.shields.io/badge/FIX-orange?style=flat-square) 🛠 **Canonical helper resolver 已迁移到 50+ SKILL**（tools-stability roadmap Phase 1）。10 个 canonical helper（`verify_papers.py`、`extract_paper_style.py`、`verify_paper_audits.sh`、`save_trace.sh`、`research_wiki.py`、`arxiv_fetch.py` + 4 个 fetcher、`verify_wiki_coverage.sh`）的所有 SKILL.md 调用方现在统一通过 [`shared-references/integration-contract.md`](skills/shared-references/integration-contract.md) §2 定义的 strict-safe 3 层链 `.aris/tools/` → `tools/` → `$ARIS_REPO/tools/` 解析 helper 路径，不再硬编码 `python3 tools/foo.py`。§2 还定义了 5 种 failure policy（A gate / B side-effect / C forensic / D1 cascade / D2 multi-source aggregate / E diagnostic），配 POSIX-sh + set-e + set-u 安全示例和 per-helper policy 分配表，下游作者不用猜。Codex skill 镜像同步。**⚠️ 现有用户**：如果 2026-04-30 之后跑过 `bash tools/install_aris.sh`（Phase 0 自动创建 `.aris/tools` 软链），无需操作；否则重跑一次（脚本幂等）。
- **2026-05-16** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🤖 **SKILL.md 硬编码 `tools/<helper>` 的 advisory CI lint**（[`.github/workflows/lint-skills-helpers.yml`](.github/workflows/lint-skills-helpers.yml) + [`tools/lint_skills_helpers.sh`](tools/lint_skills_helpers.sh)）。新 workflow 在 PR 修改 SKILL.md / shared-references 时触发，扫描 14 个 canonical helper-invocation 模式，把直接 `python3 tools/foo.py` 或 `bash tools/foo.sh` 引用报告到 GitHub Actions summary。**仅 advisory，永不卡 CI**；reviewer 可以要求迁移再合。当前 tree 有 4 个 carry-over flag（figure-spec、paper-illustration-image2 + 2 个 prose 提及），已知，留给后续 self-contained-script 迁移。
