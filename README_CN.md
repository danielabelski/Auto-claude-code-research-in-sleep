...
- **2026-05-17** — ![FIX](https://img.shields.io/badge/FIX-orange?style=flat-square) 🛠 **Tools-stability roadmap (Phase 1+2+3) 完整收尾**（closes [#176](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/176) / [#177](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/177) / [#178](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/178)）。社区反馈"`install_aris.sh` 跑完但 helper script 在用户项目里找不到"触发的 3 阶段重构，约 14 个 commit、~50 SKILL 改动。**Phase 1（resolver migration）** —— 10 个 canonical helper（`verify_papers.py`、`extract_paper_style.py`、`verify_paper_audits.sh`、`save_trace.sh`、`research_wiki.py`、`arxiv_fetch.py` + 4 个 fetcher、`verify_wiki_coverage.sh`）的所有 SKILL.md 调用方现在统一通过 [`shared-references/integration-contract.md`](skills/shared-references/integration-contract.md) §2 定义的 strict-safe 3 层链 `.aris/tools/` → `tools/` → `$ARIS_REPO/tools/` 解析 helper 路径，不再硬编码 `python3 tools/foo.py`；§2 同时定义 5 种 failure policy（A gate / B side-effect / C forensic / D1 cascade / D2 multi-source aggregate / E diagnostic），配 POSIX-sh + set-e + set-u 安全示例和 per-helper policy 分配表；Codex skill 镜像同步。**Phase 2（advisory CI）** —— 新增 [`.github/workflows/lint-skills-helpers.yml`](.github/workflows/lint-skills-helpers.yml) + [`tools/lint_skills_helpers.sh`](tools/lint_skills_helpers.sh)，PR 修改 SKILL.md 时扫描 14 个硬编码 `python3 tools/foo.py` / `bash tools/foo.sh` 模式并报告到 GitHub Actions summary，**仅 advisory，永不卡 CI**（blocking enforcement [#178](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/178) 显式 skip——deprecation cycle 在 paper-writing 时段摩擦过大；reviewer 文化是 enforcement）。**Phase 3（Arch C self-contained）** —— 3 个 single-owner helper 迁入对应 SKILL 自包含 `scripts/` 目录（matches CC 官方 layout 与社区建议）：`tools/figure_renderer.py` → `skills/figure-spec/scripts/`、`tools/paper_illustration_image2.py` → `skills/paper-illustration-image2/scripts/`、`tools/experiment_queue/{queue_manager,build_manifest}.py` → `skills/experiment-queue/scripts/`；owner SKILL 用 hybrid 链——Layer 0 `${CLAUDE_SKILL_DIR}/scripts/<helper>`（CC 1.0+ self-contained）+ Layer 1-3 canonical chain，原 `tools/` 路径保留 `os.execv` Python 转发 shim；§2 新增 "Layer 0 — self-contained owner SKILL" 子节固化 pattern 供后续 single-owner 迁移参考。Shared helper（多 SKILL 共用——`research_wiki.py`、`save_trace.sh`、`verify_paper_audits.sh`、fetchers ……）继续留在 `tools/` 作为 ARIS shared runtime——只有真正 single-owner 的 helper 才迁。**⚠️ 现有用户**：无需操作；legacy `tools/` 入口现在是 `os.execv` shim 透传到新 canonical 位置，`install_aris.sh`（Phase 0，[#174](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/174)，2026-04-30 起可用）创建的 `.aris/tools/<helper>` 软链行为一致。如果 2026-04-30 之后没跑过 `install_aris.sh`，幂等重跑一次即可全部对齐。
- **2026-05-14** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🩹 **`/paper-plan` + `/paper-write` 学会 `GAP_REPORT.md` + `<!-- DATA_NEEDED -->` 规则** ([#217](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/217))。当 `— style-ref:` 启用，且用户项目下存在结构性 assets（`figures/` / `results/` / `data/` / `tables/` / `sec/` / `NARRATIVE_REPORT.md` / `CLAIMS_FROM_RESULTS.md`）中任一时，`/paper-plan` 自动 emit **Gap Report**——把 exemplar 的 section 拓扑 + 密度要求（从 `style_profile.md`）对照用户实际 assets，暴露用户**没有证据填充**的结构性槽位（例如「exemplar 有 3×4 ablation 表，你没有 ablation 数据」）。稳定 Slot ID（`GAP_S5_ABLATION` ...）。然后 `/paper-write` 消费 Gap Report：遇到 `status: missing` 的槽位，写 `<!-- DATA_NEEDED: <Slot ID> — <描述> -->` HTML 注释**而不是编造内容**——PDF 渲染时不可见，`grep` 友好用于人审 triage / `/experiment-bridge` 后续补实验。是对默认「no placeholders」规则的窄 carve-out，只在 GAP_REPORT 列出的 missing 槽位生效。原始想法来自 [@zhangpelf](https://github.com/zhangpelf)。Stage 1（exemplar 解剖）已被 `— style-ref:`（2026-05-03）覆盖；Stage 3 真实性约束已被 `/paper-claim-audit` + `/citation-audit` + `verify_papers.py` + `/proof-checker` + `/kill-argument` + 6-态 assurance contract 覆盖——只吸收了 Gap Analysis + DATA_NEEDED markers 部分。
